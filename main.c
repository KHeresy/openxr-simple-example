...
init_openxr(xr_example* self)
{
	...
	result = xrEnumerateInstanceExtensionProperties(NULL, extensionCount, &extensionCount,
	                                                extensionProperties);
	if (!xr_result(NULL, result, "Failed to enumerate extension properties"))
		return 1;

	if (!isExtensionSupported(XR_KHR_OPENGL_ENABLE_EXTENSION_NAME, extensionProperties,
	                          extensionCount)) {
	...
	printf("Loader found %d api layers%s", apiLayerCount, apiLayerCount == 0 ? "\n" : ": ");
	...
	xrEnumerateApiLayerProperties(apiLayerCount, &apiLayerCount, apiLayerProperties);
	for (uint32_t i = 0; i < apiLayerCount; i++) {
		if (strcmp(apiLayerProperties[i].layerName, "XR_APILAYER_LUNARG_core_validation") == 0) {
			lunargCoreValidationSupported = true;
		}
		printf("%s%s", apiLayerProperties[i].layerName, i < apiLayerCount - 1 ? ", " : "\n");
	}
	...
	if (lunargCoreValidationSupported && useCoreValidationLayer) {
		...
		const char* const enabledApiLayers[] = {"XR_APILAYER_LUNARG_core_validation"};
	...
	// extension is supported, we do want to know if the system supports it.
	{
		...
		self->hand_tracking_supported = hand_tracking_extension_supported && ht.supportsHandTracking;
	...
	result =
	    xrEnumerateViewConfigurations(self->instance, systemId, 0, &viewConfigurationCount, NULL);
	...
	result = xrEnumerateViewConfigurations(self->instance, systemId, viewConfigurationCount,
	 ...
	 * does not support Stereo VR. */
	{
		...
		for (uint32_t i = 0; i < viewConfigurationCount; ++i) {
			...
			result = xrGetViewConfigurationProperties(self->instance, systemId, viewConfigurations[i],
			                                          &properties);
			if (!xr_result(self->instance, result, "Failed to get view configuration info %d!", i))
				return 1;

			if (viewConfigurations[i] == stereoViewConfigType &&
			    /* just to verify */ properties.viewConfigurationType == stereoViewConfigType) {
		...
		if (stereoViewConfigProperties.type != XR_TYPE_VIEW_CONFIGURATION_PROPERTIES) {
	...
	result = xrEnumerateViewConfigurationViews(self->instance, systemId, stereoViewConfigType, 0,
	                                           &self->view_count, NULL);
	if (!xr_result(self->instance, result, "Failed to get view configuration view count!"))
		return 1;

	self->configuration_views = malloc(sizeof(XrViewConfigurationView) * self->view_count);
	...
	result = xrEnumerateViewConfigurationViews(self->instance, systemId, stereoViewConfigType,
	...
	if (!initGLX(&self->graphics_binding_gl.xDisplay, &self->graphics_binding_gl.visualid,
	             &self->graphics_binding_gl.glxFBConfig, &self->graphics_binding_gl.glxDrawable,
	...
	// the runtime will support whatever we use
	{
		...
		result = xrEnumerateReferenceSpaces(self->session, 0, &referenceSpacesCount, NULL);
		...
		result = xrEnumerateReferenceSpaces(self->session, referenceSpacesCount, &referenceSpacesCount,
		                                    referenceSpaces);
		...
		for (uint32_t i = 0; i < referenceSpacesCount; i++) {
			if (referenceSpaces[i] == XR_REFERENCE_SPACE_TYPE_LOCAL) {
				printf("XR_REFERENCE_SPACE_TYPE_LOCAL%s", i == referenceSpacesCount - 1 ? "\n" : ", ");
				localSpaceSupported = true;
			} else if (referenceSpaces[i] == XR_REFERENCE_SPACE_TYPE_STAGE) {
				printf("XR_REFERENCE_SPACE_TYPE_STAGE%s", i == referenceSpacesCount - 1 ? "\n" : ", ");
				stageSpaceSupported = true;
			} else if (referenceSpaces[i] == XR_REFERENCE_SPACE_TYPE_VIEW) {
				printf("XR_REFERENCE_SPACE_TYPE_VIEW%s", i == referenceSpacesCount - 1 ? "\n" : ", ");
			} else {
	...
	XrReferenceSpaceCreateInfo localSpaceCreateInfo = {.type = XR_TYPE_REFERENCE_SPACE_CREATE_INFO,
	                                                   .next = NULL,
	                                                   .referenceSpaceType =
	                                                       XR_REFERENCE_SPACE_TYPE_LOCAL,
	                                                   .poseInReferenceSpace = identityPose};

	result = xrCreateReferenceSpace(self->session, &localSpaceCreateInfo, &self->local_space);
	...
	XrSessionBeginInfo sessionBeginInfo = {.type = XR_TYPE_SESSION_BEGIN_INFO,
	                                       .next = NULL,
	                                       .primaryViewConfigurationType = stereoViewConfigType};
	...
	result = xrEnumerateSwapchainFormats(self->session, 0, &swapchainFormatCount, NULL);
	...
	result = xrEnumerateSwapchainFormats(self->session, swapchainFormatCount, &swapchainFormatCount,
	                                     swapchainFormats);
	...
	for (uint32_t i = 0; i < self->view_count; i++) {
		...
		result = xrCreateSwapchain(self->session, &swapchainCreateInfo, &self->swapchains[i]);
		if (!xr_result(self->instance, result, "Failed to create swapchain %d!", i))
			return 1;

		result = xrEnumerateSwapchainImages(self->swapchains[i], 0, &swapchainLength[i], NULL);
	...
	for (uint32_t i = 0; i < self->view_count; i++) {
		...
		self->images[i] = malloc(sizeof(XrSwapchainImageOpenGLKHR) * swapchainLength[i]);
		...
		result =
		    xrEnumerateSwapchainImages(self->swapchains[i], swapchainLength[i], &swapchainLength[i],
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24,
	             self->configuration_views[0].recommendedImageRectWidth,
	             self->configuration_views[0].recommendedImageRectHeight, 0, GL_DEPTH_COMPONENT,
	             GL_UNSIGNED_INT, 0);
...
main_loop(xr_example* self)
{
	...
	XrActionSpaceCreateInfo actionSpaceInfo = {.type = XR_TYPE_ACTION_SPACE_CREATE_INFO,
	                                           .next = NULL,
	                                           .action = poseAction,
	...
	while (running) {
		...
		while (pollResult == XR_SUCCESS) {
			...
			case XR_TYPE_EVENT_DATA_INTERACTION_PROFILE_CHANGED: {
				...
				for (int i = 0; i < 2; i++) {
					XrResult res = xrGetCurrentInteractionProfile(self->session, hand_paths[i], &state);
		...
		if (self->hand_tracking_supported) {
			...
			for (int i = 0; i < 2; i++) {
				...
				result = pfnLocateHandJointsEXT(self->hand_trackers[i], &locateInfo, &joint_locations[i]);
		...
		result = xrLocateViews(self->session, &viewLocateInfo, &viewState, self->view_count,
		...
		for (int i = 0; i < hands; i++) {
			...
			result = xrLocateSpace(handSpaces[i], self->local_space, frameState.predictedDisplayTime,
			                       &hand_locations[i]);
			...
			*/
			...
			if (grabValue[i].isActive && grabValue[i].currentState > 0.75) {
				...
				XrHapticActionInfo hapticActionInfo = {.type = XR_TYPE_HAPTIC_ACTION_INFO,
				                                       .next = NULL,
				                                       .action = hapticAction,
				                                       .subactionPath = handPaths[i]};
			...
			if (leverValue[i].isActive && leverValue[i].currentState != 0) {
				printf("Lever value %d: changed %d: %f\n", i, leverValue[i].changedSinceLastSync,
				       leverValue[i].currentState);
			}
		};

		// --- Begin frame
		XrFrameBeginInfo frameBeginInfo = {.type = XR_TYPE_FRAME_BEGIN_INFO, .next = NULL};
		...
		for (uint32_t i = 0; i < self->view_count; i++) {
			...
			XrMatrix4x4f_CreateProjectionFov(&projectionMatrix, GRAPHICS_OPENGL, views[i].fov, 0.05f,
			                                 100.0f);

			const XrVector3f uniformScale = {.x = 1.f, .y = 1.f, .z = 1.f};

			XrMatrix4x4f viewMatrix;
			XrMatrix4x4f_CreateTranslationRotationScaleRotate(&viewMatrix, &views[i].pose.position,
			                                                  &views[i].pose.orientation, &uniformScale);
			...
			result =
			    xrAcquireSwapchainImage(self->swapchains[i], &swapchainImageAcquireInfo, &bufferIndex);
			...
			renderFrame(self->configuration_views[i].recommendedImageRectWidth,
			            self->configuration_views[i].recommendedImageRectHeight, projectionMatrix,
			            inverseViewMatrix, hand_locations, hand_locations_valid, joint_locations,
			            self->framebuffers[i][bufferIndex], self->depthbuffer,
			            self->images[i][bufferIndex], i, frameState.predictedDisplayTime);
			glFinish();
			XrSwapchainImageReleaseInfo swapchainImageReleaseInfo = {
			    .type = XR_TYPE_SWAPCHAIN_IMAGE_RELEASE_INFO, .next = NULL};
			result = xrReleaseSwapchainImage(self->swapchains[i], &swapchainImageReleaseInfo);
		...
		// 1 pixel = 1cm = 0.001m
		quadLayer.size.width = projection_views[0].subImage.imageRect.extent.width * 0.001f;
